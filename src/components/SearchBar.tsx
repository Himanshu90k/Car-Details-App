        ...
        <div className="relative flex flex-col items-center">
            <div className="flex justify-center mt-8">
                <form className=" flex justify-center items-center gap-5 border-2 border-solid border-customBlue rounded-45 w-84 h-11.25">
                    ...
                    {/* Search lens */}
                    <button
                        onClick={(e: React.MouseEvent<HTMLButtonElement>) => searchResults[0]? navigate(`/single-car-details/${searchResults[0]._id}`) : e.preventDefault()}
                    >
                        <svg 
                            xmlns="http://www.w3.org/2000/svg" 
                            width="34" 
            ...
            <div className="absolute z-10 top-20 w-65 h-24 border-2 border-solid border-white bg-customBlack shadow-queryCardShadow rounded-md">
                <div className="py-2 h-full px-8 flex flex-col gap-y-1 overflow-y-scroll snap-y snap-mandatory snap-always customQueryScrollbar">
                    {Array.from(searchResults, (value, index) => {
                        return (
                            <Link to={`/single-car-details/${value._id}`} key={index} className="flex w-full text-white hover:text-customRed justify-between snap-center items-center">
                                <p className="font-inter font-medium text-xs">{index + 1}</p>
                                <p className="font-inter font-medium text-xxs">{value.carNo}</p>
                            </Link>
                        )
                    })}
                </div>
